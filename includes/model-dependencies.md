...
`full` is `chronos`, `kronos`, `granite`, `moirai`, `tirex`, `toto`, `mantis`. `client`, `http`, `gpu`, `dev`, `docs`, and `all-extras` are not model families. `gpu` selects the torch index for **uv** only; pip ignores it — see [GPU](../server/source.md#gpu). Moirai's `gluonts` / `lightning` / `hydra-core` pins apply when `python_version < '3.14'`.

| extra | CPU tag | GPU tag | families | models | example |
| --- | --- | --- | --- | --- | --- |
| `server` | [`:base`](https://hub.docker.com/r/geetu040/fomo/tags?name=base) | — | Naive | 1 | `naive` |
| `hub` | [`:hub`](https://hub.docker.com/r/geetu040/fomo/tags?name=hub) | [`:hub-gpu`](https://hub.docker.com/r/geetu040/fomo/tags?name=hub-gpu) | Chronos Bolt, Chronos T5, TTM, TimesFM 2.x | 71 | `chronos-bolt-tiny` |
...
| `full` | [`:full`](https://hub.docker.com/r/geetu040/fomo/tags?name=full) | [`:full-gpu`](https://hub.docker.com/r/geetu040/fomo/tags?name=full-gpu) | all of the above | 100 | `naive` |
