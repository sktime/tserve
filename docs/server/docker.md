...
```bash
docker pull geetu040/tserve:hub
```
...
```bash
docker run --rm -p 9000:8000 geetu040/tserve:hub --model chronos-bolt
```
...
```bash
docker run --rm -p 8000:8000 geetu040/tserve:full --model moirai-2 tirex
```
...
=== "bash / zsh"
    ...
    ```bash
    docker run --rm -p 8000:8000 -v "$HOME/.cache/huggingface:/root/.cache/huggingface" geetu040/tserve:hub --model chronos-bolt ttm-r3
    ```

=== "PowerShell"

    ```powershell
    docker run --rm -p 8000:8000 -v "${env:USERPROFILE}\.cache\huggingface:/root/.cache/huggingface" geetu040/tserve:hub --model chronos-bolt ttm-r3
    ```
