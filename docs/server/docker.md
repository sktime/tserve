...
Tags, families, and an example model are on [Dependencies](../models/index.md#dependencies). `hub` is Chronos Bolt/T5, TTM, and TimesFM 2.x. `chronos`, `granite`, `moirai`, `tirex`, `tirex2`, `toto`, `mantis`, `timesfm3`, `t0`, and `tafsut` include Hub plus one more stack. `kronos` is built on `base`, not `hub`. `full` has every family. Each of those has a `*-gpu` tag. There is no `:base-gpu`.
...
`naive` downloads nothing. Every other model fetches a checkpoint on first load, so set the [token](#hugging-face-token) and [cache mount](#keep-weights-between-runs) below. All 117 models: [catalog](../models/index.md).
