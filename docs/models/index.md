...
`naive` is `NaiveForecaster` — no Hub download. Every other id pulls a Hugging Face (or equivalent) checkpoint on first load. That needs the matching [family extra](#which-extra-image), or a Docker tag that already baked it in.
...
```text
model 'chronos-2' is not loaded on this server (loaded: 'chronos-bolt-tiny', 'timesfm-2.5')
```
...
```bash
uv run fomo serve --host 0.0.0.0 --port 8000 --load-models chronos-bolt-tiny timesfm-2.5
```

Swap `model` on a [forecast](../client/http.md) between those loaded ids.
