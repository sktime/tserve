...
converters in ``tserve.types.converters`` (``coerce_request``,
``decode_request``, ``encode_response``, ``pack_envelope``), not the
sktime *converters* in ``tserve.runtime.executors.sktime.converters``.

``GET /`` serves the browser dashboard from ``tserve/server/static``,
...
``to_response`` sets ``""``). TServe has no custom exception types;
...
_ENVELOPE_CONTENT_TYPE = "application/vnd.tserve.predict+arrow"
...
def models(request: Request) -> ModelsResult:
    ...
    -------
    ModelsResult
        Currently loaded models, including ``naive``.

    See Also
    --------
    tserve.types.models.ModelsResult
        Response schema.
    tserve.runtime.bootstrap.Runtime.loaded_models
...
def stats(request: Request) -> StatsResult:
    ...
    tserve.types.models.StatsResult
        Response schema.
    tserve.logging.stats.Stats.snapshot
...
def predict(request: PredictRequest, http_request: Request) -> PredictResponse:
    ...
    ------
    HTTPException
        Status 400 when coerce or predict fails. TServe has no custom
    ...
    PredictRequest
        JSON body schema.
    tserve.types.converters.coerce_request
...
async def predict_bytes(
    ...
    ``application/vnd.tserve.predict+arrow``. Failures use the same
    ...
    -------
    fastapi.Response
        Packed ``TServe`` envelope with media type
    ...
    tserve.types.converters.decode_request
        Rebuilds ``CoercedPredictRequest`` from metadata + Arrow.
    tserve.types.converters.encode_response
        Splits the coerced response into metadata and files.
    tserve.types.converters.pack_envelope
