...
``tserve.types.converters``: native frames ↔ narwhals
↔ Arrow IPC ↔ TServe envelope.
...
from tserve.types import CoercedPredictRequest, CoercedPredictResponse
...
def from_request(
    ...
    * static features, constant columns from the first ``static`` row.
    ...
    request : CoercedPredictRequest
        ...
        ``tserve.types.models.CoercedPredictRequest`` for fields.
