...
_EXECUTOR_MODULES = {
    "sktime": "tserve.runtime.executors.sktime.executor",
    "pytorch-forecasting": "tserve.runtime.executors.pytorch_forecasting.executor",
}
...
def _ensure_registered(name: str) -> None:
    ...
    ImportError
        ...
        caller to ``pip install 'tserve[{name}]'`` (``name`` is used as
    ...
    try:
        ...
        raise ImportError(
            ...
            f"install with pip install 'tserve[{extra}]'"
        ) from exc
