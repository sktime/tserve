...
forecasts; that is ``tserve.scheduling.scheduler.Scheduler``.

See Also
--------
tserve.runtime.registry.resolve_model
    Turn a model item into ``ModelInfo``.
tserve.runtime.executors.create_executor
...
from tserve.logging import Stats
from tserve.logging.utils import format_mib, paint
from tserve.runtime.executors import Executor, create_executor
from tserve.runtime.registry import resolve_model
from tserve.scheduling.scheduler import Scheduler
from tserve.types import ModelInfo, ModelsResult
...
class Runtime:
    ...
    scheduler : Scheduler
        Dispatch handle from ``tserve.scheduling`` (not this package).
    models : dict of str to ModelInfo
        Loaded model id → listing row (``id``, ``executor``, ``source``).
    stats : tserve.logging.Stats
