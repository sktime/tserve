...
from tserve.types.models import ModelInfo, ModelsResult
...
def test_bootstrap():
    ...
    with (
        patch(
            "tserve.runtime.bootstrap.resolve_model", return_value=info
        ) as resolve_model,
        patch(
...
def test_bootstrap_loads_object():
    ...
    with (
        patch("tserve.runtime.bootstrap.resolve_model", return_value=info),
        patch("tserve.runtime.bootstrap.create_executor", return_value=executor),
    ):
