"""Wire converters: native frames ↔ narwhals ↔ Arrow IPC ↔ TServe envelope.

This module is the *wire* layer. It does not map requests onto sktime
``(y, X, fh)`` — that is ``tserve.runtime.executors.sktime.converters``.
...
def _to_narwhals(
    ...
    if isinstance(df, dict):
        try:
            ...
            raise ValueError(
                f"{name} has a column TServe could not convert to a single Arrow "
    ...
    try:
        ...
        raise TypeError(
            f"{name} is not a table TServe can read: got {type(df).__name__}.\n\n"
...
def _to_bytes(df: nw.DataFrame) -> bytes:
    ...
    bytes
        ...
        uses media type ``application/vnd.tserve.predict+arrow``.
    """
    df = df.to_arrow()
    sink = io.BytesIO()
...
def _from_arrow_bytes(blob: bytes, *, name: str) -> nw.DataFrame:
    ...
    try:
        ...
        raise ValueError(
            ...
            "empty body, a JSON body, or an Arrow *file*. The TServe client writes "
            "this format for you."
        ) from error
...
def encode_response(response: CoercedPredictResponse) -> tuple[dict, dict[str, bytes]]:
    ...
    --------
    pack_envelope
        Wraps this pair in the ``TServe`` binary envelope.
    """
    bytes_encoded = {}
...
def pack_envelope(metadata: dict, files: dict[str, bytes]) -> bytes:
    """Pack response metadata and Arrow blobs into a ``TServe`` envelope.

    Layout: magic ``b"TSRV"``, version byte ``1``, little-endian
...
def unpack_envelope(body: bytes) -> tuple[dict, dict[str, bytes]]:
    """Parse a ``TServe`` version-1 envelope into metadata and file parts.
    ...
    ------
    ValueError
        If the buffer is truncated, magic is not ``TSRV``, or the
    ...
    if body[:4] != b"TSRV":
        raise ValueError("invalid predict envelope: expected TSRV magic bytes")
    if body[4] != 1:
