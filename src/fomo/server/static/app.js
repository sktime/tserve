...
(() => {
  ...
  async function refreshModels(quiet = true) {
    ...
    try {
      ...
      if (!state.models.length) {
        ...
        $('#model-hint').textContent = 'start the server with --load-models <id> to load a model';
      } else {
  ...
  function setTheme(theme) {
    ...
    try { localStorage.setItem('fomo-theme', theme); } catch { /* ignore */ }
    renderChart();
  }
  ...
  async function init() {
    let stored = null;
    try { stored = localStorage.getItem('fomo-theme'); } catch { /* ignore */ }
